...
const ModeButton: React.FC<ModeButtonProps> = ({ label, isActive, onClick, mode }) => {
    return (
        <StyledWrapper $isActive={isActive} $mode={mode}>
...
const StyledWrapper = styled.div<{ $isActive: boolean; $mode: string }>`
  .uiverse {
    --duration: 7s;
    --easing: linear;
    
    /* Default Colors (Focus - Blue) */
    --c-color-1: ${props => props.$mode === 'focus' ? 'rgba(26, 163, 255, .7)' : props.$mode === 'shortBreak' ? 'rgba(26, 255, 163, .7)' : 'rgba(255, 26, 117, .7)'};
    --c-color-2: ${props => props.$mode === 'focus' ? '#1aff1a' : props.$mode === 'shortBreak' ? '#1affd6' : '#ff1a75'};
    --c-color-3: ${props => props.$mode === 'focus' ? '#ff1a75' : props.$mode === 'shortBreak' ? '#1aff1a' : '#ff871a'};
    --c-color-4: ${props => props.$mode === 'focus' ? 'rgba(26, 232, 255, .7)' : props.$mode === 'shortBreak' ? 'rgba(26, 255, 232, .7)' : 'rgba(255, 131, 26, .7)'};
    
    --c-shadow: ${props => props.$isActive ? (props.$mode === 'focus' ? 'rgba(59, 130, 246, 0.8)' : props.$mode === 'shortBreak' ? 'rgba(34, 197, 94, 0.8)' : 'rgba(236, 72, 153, 0.8)') : 'rgba(255, 255, 255, 0.1)'};
    --c-shadow-inset-top: rgba(255, 255, 255, 0.2);
    --c-shadow-inset-bottom: rgba(0, 0, 0, 0.2);
    
    --c-radial-inner: ${props => props.$isActive ? (props.$mode === 'focus' ? '#1e40af' : props.$mode === 'shortBreak' ? '#166534' : '#9d174d') : '#334155'};
    --c-radial-outer: ${props => props.$isActive ? (props.$mode === 'focus' ? '#1e3a8a' : props.$mode === 'shortBreak' ? '#14532d' : '#831843') : '#1e293b'};
    ...
    box-shadow: 0 0 ${props => props.$isActive ? '25px' : '10px'} var(--c-shadow);
    ...
    &:hover {
      ...
      opacity: 1;
      box-shadow: 0 0 30px var(--c-shadow);
    }
  }
  ...
  /* Keyframes inherited from user code */
